...
1. **os库**是python基础库，提供了操作系统接口，用于创建文件夹等文件系统操作

2. **torch**是整个PyTorch框架的基础，提供了底层张量（Tensor）库与自动微分（Autograd）引擎，知道这是PyTorch框架的基础即可，不必深究，我们在main.py文件中会用到其中的生成张量、梯度清零等底层方法.

3. **torch.nn**是神经网络（Neural Network）的基础，提供了 nn.Module 基类，所有的神经网络模块，都要从nn.Module 基类继承而来。torch.nn提供了构建神经网络模型最基础的模块，比如基础卷积Conv2d、基础激活函数ReLU等。此外，nn.MSELoss、nn.CrossEntropyLoss 等损失函数，同样被封装在这个库中.
...
即便知道了上文说过的6大核心逻辑，好多人在这个地方仍然会卡壳。“知道”和“会写”的小小裂缝，在这里第一次出现了。好在这个裂缝足够小，我们能很轻易的跨过去。如果想不清楚，就先不要为难自己必须面面俱到，不管怎么样先写起来。
...
再进一步，我们考虑网络结构模块继承自torch.nn.Module，必须要实现前向传播的forward方法，在其\_\_init\_\_初始化方法中，实例化并组织所有的网络模块；数据类继承自Dataset，那么我们就要创建一个自己的数据集类，从torch.utils.data中的DataSet继承，必须要实现\_\_len\_\_和\_\_getitem\_\_方法，当然，少不了的还有初始化方法\_\_init\_\_；验证评估模块和训练模块，暂时分别写成一个方法（而不是一个单独的类）。Loss计算和优化器，我们刚刚引入的库中都提供了相应的实现，我们就先用pytorch库中现成的方法。
...
class SimpleDetector(nn.Module):
    def __init__(self):
         #在这里定义模型的结构，也就是模型包含哪些模块
        pass
    def forward(self):
        ##在这里定义模型怎么处理数据，也就是依次调用哪些模型来处理数据，最终输出处理后的结果
        pass
...
接下来就是网络结构的定义。我们通过nn.Sequential把若干个pytorch提供的基础模块组合在一起，形成一个大的网络模块。nn.Sequential是一个常用的有序容器，他就像工厂里的一条流水线，数据（张量）从一端输入，按照定义好的顺序依次经过流水线上的每一个节点（神经网络模块），最后从另一端输出。
...
这里因为就1个nn.Conv2d卷积模块，所以就没有必要再使用nn.Sequential了。同样注意nn.Conv2d的参数不要写错。检测头（head）使用一个1x1的基础卷积模块，把特征图的输出维度转化成我们需要的目标格式。
