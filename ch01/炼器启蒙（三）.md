...
恭喜你，这个基础版的卷积网络模型已经写完了。

虽然它只能拟合一堆随机数，网络的深度看起来也不是很深，但全世界所有最顶尖的目标识别的卷积网络模型的底层运转逻辑，跟这一百多行代码没有任何本质区别。
...
如果不出意外的话，您会看到训练从Epoch 1进行到Epoch 20，loss呈现下降的趋势，mAP呈现上升的趋势，当然loss和mAP大概率会是振荡的，因为我们是基于随机数在进行的训练。训练过程中会根据mAP保存效果最佳的模型为best.pt，每一个Epoch都会保存一个last.pt，当然，新的last.pt会覆盖旧的last.pt，所以在根目录下的weights文件夹内，你始终只会看到最新的那一个last.pt。
...
对于训练出来的最优模型best.pt,你也可以尝试使用他，感受一下使用模型的过程。虽然这个模型目前并没有实际的作用。
